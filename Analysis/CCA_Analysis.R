...
library(dplyr)
library(ggplot2)
# Set yo# Set yo# Set your working directory to the project folder and load combootcca functions
setwd("C:/Users/colem/Desktop/STOR 674/STOR674 Final Project")
source("Analysis/combootcca_func/funcs.R")
...
ggplot(all_PC_data_x, aes(x = Index, ymin = LB, ymax = UB, color = CI_contains_zero)) +
...
ggplot(all_PC_data_y, aes(x = Index, ymin = LB, ymax = UB, color = CI_contains_zero)) +
  geom_errorbar(width = 0.2) +  # Add error bars for confidence intervals
  geom_point(aes(y = (LB + UB) / 2)) +  # Add points for the mid-point of the CI
  scale_color_manual(values = c("red", "blue")) +  # Color the points and error bars based on CI_contains_zero
  facet_wrap(~ PC, scales = "free_y",ncol = 1) + 
...
ggplot(correlation_data[c(1:30),], aes(x = PC, y = Correlation)) +
...
Xcoef_original <- Vx[,c(1:300)] %*% Dx[c(1:300),c(1:300)]^.5 %*% as.matrix(((x_first_PC$UB - x_first_PC$LB))/2)
...
Ycoef_original_TraitsCAT <-case_when(
      Ycoef_original_Traits %in% 1:45  ~ "Cognition",  # Traits 1 to 5 are "Cognition"
      Ycoef_original_Traits %in% 46:52  ~ "Motor",   # Traits 6 to 8 are "Emotion"
      Ycoef_original_Traits %in% 53:88 ~ "Substance Use",    # Traits 9 to 10 are "Health"
      ...
      TRUE ~ "Alertness"  # Any other trait number gets "Other"
    )
...
ggplot(Ycoef_original_PC1,aes(x=Ycoef_original_Index,y= Ycoef_original_PC1Values)) +
